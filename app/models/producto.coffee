...
class Producto extends Spine.Model
  ...
  @fetch_from_sf: (username,password, last_update) ->
    query = "Select Id,Name,CodigoExterno__c,Precio_Distribuidor__c, InventarioMinimo__c , Meta__c, Venta__c , Familia__c,Impuesto__c,Costo__c ,Activo__c,InventarioActual__c, DescuentoMaximo__c from Producto__c where Precio_Distribuidor__c > 0 and LastModifiedDate > "+ last_update.to_salesforce()
    data = {query: query,  username: username, password: password  }
    $.ajax
      url:"http://rodco-api.heroku.com/query"
      type: "POST"
      data: data
